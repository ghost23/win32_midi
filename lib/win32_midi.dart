...
final DynamicLibrary _nativeLib = DynamicLibrary.open('win32_midi_plugin.dll');
...
final _getMidiInDeviceCapabilities = _nativeLib
    .lookupFunction<MIDIINCAPS Function(Int32 i), MIDIINCAPS Function(int i)>('getMidiInDeviceCapabilities');

final _nOpenMidiInput = _nativeLib
    .lookupFunction<Int32 Function(Int32 port, Int64 callbackPort), int Function(int port, int callbackPort)>('openMidiInput');
...
class Win32Midi {
  ...
  late StreamSubscription<dynamic> portSubscription;
  ...
  int closeMidiInput() {
    if(portSubscription != null) portSubscription.cancel();
    receivePort.close();
...
enum MidiMessageType {
  OPEN, CLOSE, DATA, ERROR, UNKNOWN
}

enum MidiStatusFlag {
  NoteOff, NoteOn, PolyAftertouch, CC, PC, MonoAftertouch, PitchBend, System
}
...
class MidiData {
  ...
  MidiData(this.status, this.channel, {this.note, this.value, this.controller, this.program});
  ...
  factory MidiData.fromFourByteInt(int fourByteInt) {
    ...
    return MidiData(status, channel, note: note, value: value, controller: controller, program: program);
  }
...
class MidiMessage {
  ...
  factory MidiMessage.fromNativeMessage(List<int> message) {
    return MidiMessage(
      messageEnumMap.containsKey(message[0]) ? messageEnumMap[message[0]]! : MidiMessageType.UNKNOWN, 
      message[1],
      message[3],
